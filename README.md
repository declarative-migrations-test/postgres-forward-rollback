...
The `fixtures/canonical-quote-v1.sql` scenario is a byte-identical snapshot of `canonical-cloud/canonical-api-server.rs@f57528ccf2b077644917c1f770c97eca3027b8e7:db/schema.sql`. Its provenance file records the source Git blob and the exact declarative migrator revision.
...
- the four owner policies, five required indexes, two update triggers, and trigger function exist;
- exactly one active `canonical_context` row is permitted per owner while historical inactive rows remain valid;
...
All data and identifiers are synthetic. The workflow has no production database, Cloudflare, Supabase, R2, Kubernetes, Gemini, or secret-store access.
